...
contract Vote is ReentrancyGuard {
    ...
    function initializeVote(address _vabbleDAO, address _stakingPool, address _property) external onlyAuditor {
    ...
    function voteToFilms(uint256[] memory _filmIds, uint256[] memory _voteInfos) external onlyStaker initialized nonReentrant {
    ...
    function voteToAgent(
        ...
        if (_flag == 1) {
            ...
            if (av.disputeVABAmount == 0) {
                ...
                require(
                    __isVotePeriod(IProperty(DAO_PROPERTY).disputeGracePeriod(), av.disputeStartTime),
                    "agent elapsed grace period"
                );
            }
        } else {
    ...
    function replaceAuditor(address _agent) external onlyStaker nonReentrant {
        ...
        if (disputeTime > 0) {
            ...
            require(
                !__isVotePeriod(
                    IProperty(DAO_PROPERTY).agentVotePeriod() + IProperty(DAO_PROPERTY).disputeGracePeriod(),
                    pCreateTime
                ),
