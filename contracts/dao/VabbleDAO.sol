...
contract VabbleDAO is ReentrancyGuard {
    ...
    function __setFinalFilm(address _user, uint256 _filmId, uint256 _watchPercent, uint256 _rentPrice) private {
        Film memory fInfo = filmInfo[_filmId];
        require(
            fInfo.status == Helper.Status.APPROVED_LISTING || fInfo.status == Helper.Status.APPROVED_WITHOUTVOTE,
    ...
    function getFilmFund(uint256 _filmId) public view returns (uint256 raiseAmount_, uint256 fundPeriod_, uint256 fundType_) {
