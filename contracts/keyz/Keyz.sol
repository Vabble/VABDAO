...
//     event SaleSettled(uint256 indexed saleId, uint256 indexed totalAmount);
...
//     event VabbleShareUpdated(uint256 indexed newShare);
//     event DaoShareUpdated(uint256 indexed newShare);
//     event MinIpOwnerShareUpdated(uint256 indexed newShare);
//     event PercentagePrecisionUpdated(uint256 indexed newPrecision);
//     event VabbleAddressUpdated(address indexed newAddress);
//     event DaoAddressUpdated(address indexed newAddress);
//     event IpOwnerAddressUpdated(address indexed newAddress);

//     event MaxDurationInMinutesUpdated(uint256 indexed newMaxDuration);
//     event MinBidIncrementAllowedUpdated(uint256 indexed newMinBidIncrement);
//     event MaxBidIncrementAllowedUpdated(uint256 indexed newMaxBidIncrement);

//     event MaxRoomKeysUpdated(uint256 indexed newMaxRoomKeys);
//     event PendingReturnsWithdrawn(address indexed user, uint256 indexed amount);
...
//     ) {
//         // Add zero-address checks for all address parameters
//         require(_vabbleAddress != address(0), "Vabble address cannot be zero");
//         require(_vabTokenAddress != address(0), "VAB token address cannot be zero");
//         require(_daoAddress != address(0), "DAO address cannot be zero");
//         require(_uniHelper != address(0), "UniHelper address cannot be zero");
...
//     function settleSale(uint256 saleId) external nonReentrant saleExists(saleId) whenNotPaused {
//         Sale storage sale = sales[saleId];
...
//         // Add access control
//         require(
//             msg.sender == sale.roomOwner || msg.sender == owner() || msg.sender == sale.ipOwnerAddress,
//             "Unauthorized: only room owner, contract owner, or IP owner can settle"
//         );
...
//         // Set settled before transfers to prevent reentrancy
//         sale.settled = true;
...
//         _safeTransfer(sale.ipOwnerAddress, ipOwnerAmount, "IP Owner transfer failed"); // Use sale-specific
//             // ipOwnerAddress
...
//         uint256[] memory amountsReceived = IUniswapV2Router02(UNISWAP_ROUTER).swapExactETHForTokens{
//             value: amountToPool
//         }(slippageTolerance, path, address(this), block.timestamp + 5);

//         uint256 vabAmount = amountsReceived[1];

//         IERC20 vabToken = IERC20(vabTokenAddress);
//         uint256 currentAllowance = vabToken.allowance(address(this), STAKING_POOL);

//         if (currentAllowance < vabAmount) {
//             // First reset allowance to zero
//             if (currentAllowance > 0) {
//                 vabToken.safeDecreaseAllowance(STAKING_POOL, currentAllowance);
//             }
//             // Then increase allowance to the exact amount needed
//             vabToken.safeIncreaseAllowance(STAKING_POOL, vabAmount);
//         }
...
//     function claimRefund(uint256 saleId, uint256 keyId) external nonReentrant saleExists(saleId) whenNotPaused {
//         Sale storage sale = sales[saleId];
//         require(block.timestamp > sale.endTime, "Sale not ended");
//         require(!sale.settled, "Sale already settled");
...
//         // Clear the pending returns before transfer to prevent reentrancy
