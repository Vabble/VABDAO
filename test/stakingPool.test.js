...
describe('StakingPool', function () {
  ...
  it('AddReward and WithdrawReward with VAB token', async function() {
    ...
    const tx = await this.StakingPool.connect(this.customer1).withdrawReward({from: this.customer1.address})
    this.events = (await tx.wait()).events
    ...
    const tx_new = await this.StakingPool.connect(this.customer1).withdrawReward({from: this.customer1.address})
