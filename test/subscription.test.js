...
describe('Subscription', function () {
  ...
  beforeEach(async function () {
    ...
    await expect(
      this.SubContract.addStudio(this.studio1.address)
    ).to.emit(this.SubContract, 'StudioAdded').withArgs(this.auditor.address, this.studio1.address);    
    await this.SubContract.connect(this.auditor).addStudio(this.studio2.address, {from: this.auditor.address})
  ...
  it('2. NFT Gated Content', async function () {
    ...
    const contentData1 = getUploadGateContent(approved_ids[0], NFTs.addressList, NFTs.tokenIdList, NFTs.tokenTypeList)
    const contentData2 = getUploadGateContent(approved_ids[1], NFTs.addressList, NFTs.tokenIdList, NFTs.tokenTypeList)
    const contentData3 = getUploadGateContent(approved_ids[2], NFTs.addressList, NFTs.tokenIdList, NFTs.tokenTypeList)
