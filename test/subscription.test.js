...
const ERC20 = require(path.join(__dirname, '../data/ERC20.json'));
const { CONFIG, DISCOUNT, ZERO_ADDRESS, getBigNumber } = require(path.join(__dirname, '../scripts/utils'));
...
describe('Subscription', function () {
  ...
  it('1. Subscription by token based on Period', async function () {
    ...
    // Get initial staking pool VAB balance
    const initialStakingPoolBalance = await this.vabToken.balanceOf(this.StakingPool.address);

    // Activate subscription with VAB
    const tx = await this.Subscription.connect(this.customer1).activeSubscription(
      this.vabToken.address,
      periodVal,
      { from: this.customer1.address }
    );

    // Get events and verify subscription activation
    const receipt = await tx.wait();
    expect(receipt.events.some(e => e.event === 'SubscriptionActivated')).to.be.true;

    // Verify subscription info
    const { activeTime, period, expireTime } = await this.Subscription.subscriptionInfo(this.customer1.address);
    expect(period).to.equal(periodVal);

    // Verify VAB was deposited to StakingPool
    const finalStakingPoolBalance = await this.vabToken.balanceOf(this.StakingPool.address);
    expect(finalStakingPoolBalance.sub(initialStakingPoolBalance)).to.equal(subAmount_vab);

    // Verify subscription is active
    const isActived = await this.Subscription.isActivedSubscription(this.customer1.address);
    expect(isActived).to.be.true;

    // Test additional subscription period
    await network.provider.send('evm_increaseTime', [4 * 86400]); // 4 days
    await network.provider.send('evm_mine');

    const additionalPeriod = 2;
    const additionalAmount = await this.Subscription.getExpectedSubscriptionAmount(this.vabToken.address, additionalPeriod);

    const initialBalance2 = await this.vabToken.balanceOf(this.StakingPool.address);

    await this.Subscription.connect(this.customer1).activeSubscription(
      this.vabToken.address,
      additionalPeriod,
      { from: this.customer1.address }
    );

    const finalBalance2 = await this.vabToken.balanceOf(this.StakingPool.address);
    expect(finalBalance2.sub(initialBalance2)).to.equal(additionalAmount);

    // Verify total period is updated
    const updatedInfo = await this.Subscription.subscriptionInfo(this.customer1.address);
    expect(updatedInfo.period).to.equal(periodVal + additionalPeriod);

    // Test subscription expiration
    await network.provider.send('evm_increaseTime', [200 * 86400]); // 200 days
    await network.provider.send('evm_mine');

    expect(await this.Subscription.isActivedSubscription(this.customer1.address)).to.be.false;
  });
  ...
  it('test subscription with VAB, MATIC, USDC, USDT', async function () {
    ...
    // Test VAB subscription amount and staking
    let subAmount_vab = await this.Subscription.getExpectedSubscriptionAmount(this.vabToken.address, periodVal);
    const initialStakingBalance = await this.vabToken.balanceOf(this.StakingPool.address);
    ...
    const finalStakingBalance = await this.vabToken.balanceOf(this.StakingPool.address);
    expect(finalStakingBalance.sub(initialStakingBalance)).to.equal(subAmount_vab);

    // Test other tokens (should still convert to VAB and stake)
    ...
    expect(subAmount_usdc).to.be.gt(0);
    expect(subAmount_matic).to.be.gt(0);
    expect(subAmount_usdt).to.be.gt(0);
  });
});
